    ...
    return (
        <div className="min-h-[400px] shadow-md rounded-md overflow-hidden flex flex-col cursor-pointer" onClick={handleRoute}>
            <div className="relative h-[250px] w-full">
                <Image layout="fill" src={`/images/${blog.imageBlog}`} alt={blog.title} objectFit="cover"/>
            </div>
                ...
                <div className="flex items-center justify-between mt-5">
                    <div className="flex items-center space-x-3">
                        <div className="relative rounded-full h-6 w-6">
                            <Image src={`/images/${blog.imgProfile}`} layout="fill" objectFit="cover" className="rounded-full"/>
                        </div>
